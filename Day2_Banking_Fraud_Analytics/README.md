# Day 2: Banking and Fraud Detection Analytics
...
### 1. Dataset Overview Verification
```sql
...
- Total Amount Transacted: Rs 747,555.57
- Average Transaction Amount: Rs 297.59

---

### 2. Transaction Type Breakdown (Debit vs Credit)
```sql
...
```
| TransactionType | Total Transactions | Total Amount (Rs) |
|---|---|---|
| Debit | 1,944 | Rs 573,463.00 |
| Credit | 568 | Rs 174,092.57 |

---

### 3. Banking Channel Analysis
```sql
SELECT Channel,
       COUNT(*) AS total_transactions
FROM bank_transactions
...
### 4. Customer Occupation Spending Breakdown
```sql
...
ORDER BY total_amount DESC;
```
| CustomerOccupation | Total Amount (Rs) |
|---|---|
| Student | Rs 205,786.03 |
| Doctor | Rs 184,693.81 |
| Engineer | Rs 180,650.06 |
| Retired | Rs 176,425.67 |

---

### 5. Security and Fraud Risk Audit: Login Attempts Distribution
```sql
...
### 6. High Risk Transactions Audit (LoginAttempts > 3)
```sql
SELECT *
FROM bank_transactions
...
### 7. Average Login Attempts by Channel
```sql
SELECT Channel,
       AVG(LoginAttempts)
FROM bank_transactions
...
## Key Business and Security Insights
...
4. Top Demographic Spenders: Students represent the highest total expenditure (Rs 205,786.03), closely followed by Doctors and Engineers.
...
Today I completed Day 2 by building a PostgreSQL database (2,512 Bank Transactions) and executing analytical and fraud detection queries.
...
Insights Uncovered:
- Debit Preference: 76.7% of volume comes from Debit transactions (Rs 573k+).
...
- Average spend across 2,512 transactions stands at Rs 297.59.

GitHub Repository:
https://github.com/Nirrajkadam/11_Days_11_SQL_Problems/tree/main/Day2_Banking_Fraud_Analytics
...
- Database Created (day2_bank_analysis)
- Table Created (bank_transactions)
- 2,512 Rows Imported via \copy
- All Core & Fraud Queries Executed
